...
# taken from https://raw.githubusercontent.com/quarkdown-labs/get-quarkdown/refs/heads/main/install.sh
# modified to install only necessary dependencies (i.e. not nodejs, npm and puppeteer)
...
if ! command -v java &>/dev/null; then
  ...
  if $USE_PM; then
    ...
    if [[ -z "$PM" ]]; then
      ...
      case "$PM" in
        apt) install_with_pm "$PM" openjdk-21-jdk ;;
        dnf|yum|zypper) install_with_pm "$PM" java-21-openjdk ;;
        pacman) install_with_pm "$PM" jdk21-openjdk ;;
        brew) install_with_pm "$PM" openjdk@21 ;;
      esac
    fi
  fi
